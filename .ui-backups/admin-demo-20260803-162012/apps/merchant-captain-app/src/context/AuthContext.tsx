...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (appConfig.allowDemoMode) {
      const isCaptainDemo = appConfig.demoRole === 'CAPTAIN';
      const demoRole = isCaptainDemo ? 'CAPTAIN' : 'MERCHANT';
      const demoUserId = isCaptainDemo
        ? 'c7f4b0c9-88c8-4f09-8aa5-f6c587c62ee1'
        : 'd3b07384-d113-4e4e-9c8e-3d8e3d8e3d8e';
      ...
      const mockUser = {
        ...
        user_metadata: {
          full_name: isCaptainDemo ? 'Demo Captain' : 'Demo Merchant',
          role: demoRole,
        },
        aud: 'authenticated',
      ...
      setUser(mockUser);
      setRole(demoRole);
      setActiveRole(isCaptainDemo ? 'CAPTAIN' : 'PROVIDER');
      setProviderId(isCaptainDemo ? null : demoUserId);
      setCaptainId(isCaptainDemo ? demoUserId : null);
