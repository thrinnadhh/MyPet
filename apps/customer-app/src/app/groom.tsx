...
export default function GroomScreen() {
  ...
  const fetchSalons = async () => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/discovery/providers?longitude=${coords.longitude}&latitude=${coords.latitude}&radius=10.0&type=GROOMING_CENTER`,
  ...
  const handleConfirmPayment = useCallback(async () => {
    ...
    try {
      ...
      // Offline demo — show success anyway
    }
    setPhase('success');
