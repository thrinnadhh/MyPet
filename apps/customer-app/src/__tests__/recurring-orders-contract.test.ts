...
describe('recurring order contract', () => {
  ...
  it('requires the scheduler to create an idempotent operational order', () => {
    ...
    const migration = source('../../backend/order-service/src/main/resources/db/migration/V1002__recurring_order_execution.sql');
    expect(backend).toMatch(/RecurringOrderGenerated/);
    expect(backend).toMatch(/orderService\.createOrder/);
