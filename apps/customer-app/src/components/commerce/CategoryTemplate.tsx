...
export function CategoryTemplate({
  ...
  const isFoodCatalog = useMemo(
    () =>
      ...
      products.some((product) => product.category === 'food'),
    [products, title],
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (isFoodCatalog && selectedFoodFilter !== 'ALL') {
      if (selectedFoodFilter === 'DRY' || selectedFoodFilter === 'WET') {
        list = list.filter(
          (product) => product.foodForm === selectedFoodFilter,
        );
      } else {
        list = list.filter((product) =>
          product.lifeStages?.includes(selectedFoodFilter),
        );
      }
    }
  ...
  const utilityFilters = [
    ...
    },
    {
      ...
      onPress: () =>
        setSelectedSort(
          selectedSort === 'PRICE_LOW' ? 'RELEVANCE' : 'PRICE_LOW',
        ),
    },
    {
      id: 'HIGH',
      ...
      onPress: () =>
        setSelectedSort(
          selectedSort === 'PRICE_HIGH' ? 'RELEVANCE' : 'PRICE_HIGH',
        ),
    },
    {
      id: 'RATING',
      ...
      onPress: () =>
        setSelectedSort(
          selectedSort === 'RATING' ? 'RELEVANCE' : 'RATING',
        ),
    },
    ...brands.map((brand) => ({
      id: `brand-${brand}`,
  ...
  return (
    <ScreenShell
      ...
      header={
        <ScreenHeader
          title={title}
          subtitle={subtitle ?? 'Same-day local delivery'}
        />
      }
    ...
    >
      ...
      {filteredProducts.length === 0 ? (
        ...
        <FlatList
          ...
          columnWrapperStyle={
            columns > 1 ? styles.columnRow : undefined
          }
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                onPress={() =>
                  router.push(
                    `/commerce/product-detail?id=${item.id}` as never,
                  )
                }
                accessibilityRole="button"
                accessibilityLabel={`${item.name}. ${item.brand}. ₹${item.price}. ${
              ...
              >
                ...
                >
                  ...
                  <Pressable
                    onPress={() =>
                      void toggleFavourite('PRODUCT', item.id)
                    }
                    accessibilityRole="button"
                    accessibilityLabel={
                      favourite
                  ...
                  >
                    <AppIcon
                      name="heart"
                      color={
                        favourite ? theme.danger : theme.textSecondary
                      }
                      size={20}
                    />
                  </Pressable>
