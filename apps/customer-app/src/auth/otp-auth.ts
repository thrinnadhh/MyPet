...
export type OtpErrorCode = 'INVALID_INPUT' | 'INVALID_CODE' | 'EXPIRED_CODE' | 'RATE_LIMITED' | 'NETWORK' | 'CANCELLED' | 'UNKNOWN';

export class OtpAuthError extends Error {
  constructor(public readonly code: OtpErrorCode, message: string) { super(message); }
}
...
export function normalizeOtpError(error: unknown): OtpAuthError {
  ...
  if (status === 429 || message.includes('rate') || message.includes('too many')) return new OtpAuthError('RATE_LIMITED', 'Too many attempts. Try again later.');
  if (message.includes('expired')) return new OtpAuthError('EXPIRED_CODE', 'This code has expired. Request a new code.');
  if (message.includes('invalid') || message.includes('token')) return new OtpAuthError('INVALID_CODE', 'The code is invalid. Check it and try again.');
  if (message.includes('network') || message.includes('fetch')) return new OtpAuthError('NETWORK', 'Network unavailable. Check your connection and retry.');
...
export function normalizeEmail(value: string): string {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new OtpAuthError('INVALID_INPUT', 'Enter a valid email address.');
  return email;
}
