...
- Add route role guards for provider approval, catalog writes, order status changes, appointment status changes, payout calculation, promotions, dispatch responses, and billing.
- Services should treat gateway headers as internal context, not user input. Request bodies should not be allowed to choose `customerId`, `ownerUserId`, `staffId`, or privileged role.
